...
## Purpose
Document the frontend flow, form shape, validation, and API calls required to implement "Update Profile" in the web/mobile client.
...
Notes:
- The project normalizes avatar paths using `PublicImage::normalize`.
- The user model stores an `avatar` string (path or URL).
- The backend returns `avatar_url` in the profile payload.

Additional endpoints provided for reference:
- `POST /api/author/upload`
- `POST /api/author/books/upload`
...
Single-request flow:
1. Submit the profile update as multipart/form-data to `POST /api/me/profile` including the file under `avatar` or `avatar_file`.
...
```json
{
  ...
  "data": {
    "avatar": "avatars/abcd1234.jpg",
    "avatar_url": "https://.../storage/avatars/abcd1234.jpg"
  }
}
```
...
await axios.put('/api/me/profile', {
  ...
  bio,
  facebook_url,
  avatar: avatarPathOptional
});
```

Error handling:
- Map backend validation errors returned as JSON to field-level messages.
- Backend uses a structure like `{ success: false, message: '...', errors: {...} }`.
...
## Files and Components Suggestion
